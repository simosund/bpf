...
static void *load_module_thread(void *p)
{
	...
	if (!ASSERT_NEQ(load_bpf_testmod(), 0, "load_module_thread must fail"))
...
static void test_bpf_mod_race_config(const struct test_config *config)
{
	...
	if (!ASSERT_OK(unload_bpf_testmod(), "unload bpf_testmod"))
		goto end_mmap;
...
end_module:
	unload_bpf_testmod();
	ASSERT_OK(load_bpf_testmod(), "restore bpf_testmod");
end_mmap:
